<h1>Weather App using openWeatherMap API</h1>
<br />
<img width="100%" src="https://grist.org/wp-content/uploads/2013/06/supercell1.gif" />
<p>This is a academic project to learn more about HTML, CSS, <a href="https://www.javascript.com">javaScript</a> and APIs and how to use all these technologies collectively to get the required response from the API and use the data and show it to the user.</p>
<ul>
  <p>Technologies used in this project</p>
  <br />
  <li>HTML</li>
  <li>CSS and <a href="https://unsplash.com/">Unsplash Backgrounds</a></li>
  <li><a href="https://www.javascript.com">javaScript</a></li>
  <li><a href="https://openweathermap.org">openWeatherMap API</a></li>
  <li>GitHub Pages</li>
</ul>
<br />
<br />

<h3>Researched and created by :-</h3>
<h4>Anuj Sharma</h4>
<h5><a href="mailto:21001003015@jcboseust.ac.in">Email</a></h5><br />
<h4>Kartik Chauhan</h4>
<h5><a href="mailto:21001003065@jcboseust.ac.in">Email</a></h5><br />

<h6>Students of J.C. Bose University of Science and Technology, YMCA, Faridabad, Haryana</h6>
